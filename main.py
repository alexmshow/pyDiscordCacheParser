...
PATH_TO_FILE = os.path.dirname(os.path.abspath(__name__)) + "\\"
PATH_TO_CACHE = os.path.expanduser("~") + "\\AppData\\Roaming\\discord\\Cache\\"
...
files.remove("data_0"); files.remove("data_1"); files.remove("data_2"); files.remove("data_3"); files.remove("index")
print('Starting algorithm to determine mime...')
for FILE in files:
    with open(PATH_TO_CACHE + FILE, "rb") as file:
        info = get(file.read(128))
        if info.extension.__len__() == 1:
            folder = ""
            if info.extension[0] in ["png", "jpg", "gif"]:
                folder = "images\\"
            elif info.extension[0] in ["mp4", "webm", "m4v"]:
                folder = "video\\"
